...
class MetaCorgiSnacks
  ...
  def method_missing(snack_name, *args)
    # Your code goes here...
    info = @snack_box.send("get_#{snack_name}_info", @box_id)
    tastiness = @snack_box.send("get_#{snack_name}_tastiness", @box_id)
    snack_name = "#{snack_name.to_s.split('_').map(&:capitalize).join(' ')}"
    result = "#{snack_name}: #{info}: #{tastiness} "
    tastiness > 30 ? "* #{result}" : result
  end


  def self.define_snack(snack_name)
    # Your code goes here...
    info = @snack_box.send("get_#{snack_name}_info", @box_id)
    tastiness = @snack_box.send("get_#{snack_name}_tastiness", @box_id)
    display_name = "#{snack_name.split('_').map(&:capitalize).join(' ')}"
